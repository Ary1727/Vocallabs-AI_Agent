...
SELECT
    ...
    NULL::integer AS quota_used,
    NULL::timestamp with time zone AS quota_period_start,
    NULL::integer AS quota_remaining,
    NULL::bigint AS completed_runs_total,
    NULL::numeric AS avg_run_duration_seconds;
...
ALTER TABLE ONLY public.watched_events
    ADD CONSTRAINT watched_events_pkey PRIMARY KEY (id);
...
CREATE INDEX idx_step_runs_run ON public.step_runs USING btree (workflow_run_id);
CREATE INDEX idx_watched_events_org ON public.watched_events USING btree (org_id);
CREATE INDEX idx_workflow_results_org ON public.workflow_results USING btree (org_id);
...
ALTER TABLE ONLY public.watched_events
    ADD CONSTRAINT watched_events_org_id_fkey FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.workflow_results
    ADD CONSTRAINT workflow_results_org_id_fkey FOREIGN KEY (org_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
ALTER TABLE ONLY public.workflow_results
